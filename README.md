...
- 解析結果は1大学分(1エントリ)の解析が完了するたびに逐次保存されるため、
  処理を途中で止めても、それまでに完了した大学分の問題は失われず検索できます。
  ただし、止めた瞬間に解析中だった1大学分のデータは保存されず、そのPDFの
  ステータスは「処理中」のまま残ります。続きから自動的に再開する機能は無いため、
  残りのページを取り込みたい場合は、そのPDFを削除してから再アップロードして
  ください(既に保存済みの問題は別のPDFとして重複するので、必要に応じて
  「検出結果の確認・修正」タブから重複分を削除してください)。
